...
export class DatabaseService {
  ...
  addUser(name, lastname, email, type_skin, birth_date, img, password) {
    let data = [name, lastname, email, type_skin, birth_date, img, password];
    console.log("Solicitud de agregar usuario.");
    console.log(data);
    return this.database
      .executeSql(
        "INSERT INTO user (name, lastname, email, type_skin, birth_date, img, password) VALUES (?, ?, ?, ?, ?, ?, ?)",
        data
      )
      .then(data => {
  ...
  updateUser(user: User) {
    ...
    return this.database
      .executeSql(
        `UPDATE user SET name = ?, lastname = ?, email = ?, type_skin = ?, birth_date = ?, img = ?, password =? WHERE id = ${
          user.id
        }`,
        data
      )
